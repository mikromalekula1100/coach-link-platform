...
**Ответственность:** генерация текстовых рекомендаций, анализа и сводок по тренировочному процессу с помощью локальной LLM.

### 9.1 Архитектура

```
                                   ┌→ Analytics Service (статистика спортсмена)
Тренер → API Gateway → AI Service ─┼→ Training Service  (отчёты, список спортсменов)
...
2. Собирает данные из других сервисов (Analytics и/или Training — зависит от типа запроса)
3. Формирует промпт на русском языке с конкретными данными
4. Отправляет промпт в Ollama API (`POST /api/chat`, `stream: false`)
5. Возвращает сгенерированный текст

### 9.2 Три режима работы

#### Рекомендации (`POST /ai/athletes/{id}/recommendations`)
- **Источник данных:** Analytics Service → сводка по спортсмену (объёмы, нагрузка, ЧСС, дистанция, % выполнения)
- **Системный промпт:** "Ты — ассистент тренера по лёгкой атлетике. Дай рекомендации по корректировке тренировочного процесса."
- **Назначение:** помочь тренеру скорректировать нагрузку для конкретного спортсмена

#### Анализ (`POST /ai/athletes/{id}/analysis`)
- **Источник данных:** Analytics Service → сводка + последние отчёты спортсмена
- **Системный промпт:** "Проанализируй тренировочный процесс спортсмена, выяви тенденции и проблемы."
- **Назначение:** выявить тренды — растёт ли нагрузка, снижается ли пульс, есть ли признаки перетренированности

#### Суммаризация (`POST /ai/coach/summary`)
- **Источник данных:** Training Service → текстовые отчёты **всех** спортсменов за период (по умолчанию 7 дней)
- **Системный промпт:** "Сделай краткую сводку: общие тенденции, на что обратить внимание, кому нужна корректировка нагрузки, есть ли жалобы."
- **Назначение:** когда у тренера 15–20 спортсменов и за неделю накопилось 30–40 отчётов, LLM выделяет ключевое

### 9.3 Промпты

Все промпты захардкожены в `services/ai-service/internal/service/service.go`. Каждый состоит из двух частей:
- **Системный промпт** — роль LLM ("ассистент тренера по лёгкой атлетике") и формат ответа
- **User prompt** — конкретные данные: числовые метрики и/или текстовые комментарии из отчётов
...
| POST | `/api/v1/ai/athletes/{id}/analysis` | Анализ тренировочного процесса спортсмена |
| POST | `/api/v1/ai/coach/summary` | Сводка по отчётам всех спортсменов за период |

**Тело запроса recommendations/analysis (опционально):**
```json
{"context": "Подготовка к соревнованиям на 800м через 2 месяца"}
```

**Тело запроса summary (опционально):**
```json
{
  "date_from": "2026-04-01",
  "date_to": "2026-04-13",
  "context": "Обрати внимание на жалобы на самочувствие"
}
```

**Ответ recommendations/analysis:**
```json
{
  "athlete_id": "uuid",
...
| POST  | /api/v1/ai/athletes/{id}/analysis               | Анализ тренировок от LLM (тренер)|
| POST  | /api/v1/ai/coach/summary                        | Сводка по всем спортсменам (LLM) |
