...
**Ответственность:** генерация рекомендаций и анализ тренировочного процесса с помощью локальной LLM.

### 9.1 Архитектура

```
Тренер → API Gateway → AI Service → Analytics Service (данные)
                                   → Ollama (генерация текста)
```

AI Service — это оркестратор, который:
1. Получает запрос от тренера (выбранный спортсмен + опциональный контекст)
2. Запрашивает сводку и историю тренировок из Analytics Service
3. Формирует промпт на русском языке с конкретными данными спортсмена
4. Отправляет промпт в Ollama API (`POST /api/chat`)
...
**Ускорение на Apple Silicon:** для полного использования GPU (Metal) рекомендуется установить Ollama нативно (`brew install ollama`) и указать в docker-compose переменную `OLLAMA_URL: http://host.docker.internal:11434` для ai-service.

### 9.3 Эндпоинты (только для тренера)

| Метод | Путь | Описание |
|-------|------|----------|
| POST | `/api/v1/ai/athletes/{id}/recommendations` | Рекомендации по тренировкам |
| POST | `/api/v1/ai/athletes/{id}/analysis` | Анализ тренировочного процесса |
