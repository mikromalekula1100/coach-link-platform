...
CoachLink — открытая микросервисная платформа для автоматизации тренировочного процесса и взаимодействия в системе «тренер — спортсмен».

Текущая предметная реализация ориентирована на лёгкую атлетику: это упрощает первую версию платформы и соответствует спортивной экспертизе автора проекта. При этом базовые сущности платформы — пользователи, связи тренер-спортсмен, группы, тренировочные планы, назначения, отчёты, уведомления и аналитика — не привязаны жёстко к одному виду спорта и могут быть расширены под другие дисциплины.
...
└──────┬──────────┬──────────┬──────────┬─────────────────────────┘
       ...
       ▼          ▼          ▼          ▼
┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────────┐ ┌───────────┐ ┌──────────┐
│   Auth   │ │   User   │ │ Training │ │ Notification │ │ Analytics │ │    AI    │
...
└────┬─────┘ └────┬─────┘ └────┬─────┘ └──────┬───────┘ └─────┬─────┘ └────┬─────┘
     ...
     ▼            ▼            ▼               ▼               │            ▼
┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────────┐       │       ┌──────────┐
│ auth_db  │ │ user_db  │ │training_ │ │notification_ │       │       │  Ollama  │
│(Postgres)│ │(Postgres)│ │   db     │ │     db       │       │       │  (LLM)   │
│          │ │          │ │(Postgres)│ │  (Postgres)  │       │       └──────────┘
└──────────┘ └──────────┘ └──────────┘ └──────────────┘       │
                    ▲            ▲               ▲             │
                    │            │               │    (HTTP: internal API)
...
5. **Расширяемость** — архитектура спроектирована с учётом будущих модулей (аналитика, Backend-Driven UI, расширенные AI-сценарии).
...
| Опциональное кеширование | Redis 7+ (поднят в инфраструктуре, не используется в текущей backend-логике) |
...
|----------------------|-----------------------------|--------|
| Оркестрация (dev)    | Docker Compose              | реализовано |
| CI/CD                | GitHub Actions              | реализовано |
| Оркестрация (prod)   | Docker Compose / Kubernetes | план |
| Мониторинг           | Prometheus + Grafana        | **реализовано** (см. §7.4) |
| Трассировка          | OpenTelemetry + Jaeger      | план |
...
- Rate limiting (планируется как направление развития)
- CORS-заголовки
...
- `coachlink.user.registered` — при успешной регистрации (user_id, login, full_name, email, role)
- `coachlink.user.updated` — при обновлении профиля (если добавим эту функцию в Auth)
...
**События (подписан):**
- `coachlink.user.registered` — создаёт запись в `user_profiles`
...
- **Синхронизации данных** — Auth Service публикует `coachlink.user.registered`, User Service подписан и создаёт профиль.
...
### 7.4 Наблюдаемость (Prometheus + Grafana)

Все 7 сервисов инструментированы единым middleware `pkg/httpmetrics` (отдельный
Go-модуль, подключается через `replace`-директиву — тот же приём, что и для
`pkg/events`). Каждый сервис экспортирует метрики на `/metrics` в формате
Prometheus.

**Метрики модели RED** (Rate, Errors, Duration):
...
- метка `route` берётся из **шаблона маршрута** Echo (`c.Path()`, например
  `/api/v1/training/assignments/:assignmentId`), а не из сырого URL — иначе
  кардинальность временных рядов взрывается от идентификаторов;
- `service` задаётся как `ConstLabel` при создании middleware, поэтому в PromQL
  доступна привычная селекция `{service="auth-service"}`;
- middleware врезается в API Gateway **до** JWT-проверки, чтобы отклонённые
  (401) запросы тоже попадали в метрики; эндпоинты `/health` и `/metrics`
  исключены из аутентификации;
- для ai-service заданы расширенные бакеты гистограммы (до 300 с), так как вызовы
  локальной LLM идут десятки–сотни секунд.

**Сбор и визуализация.** Prometheus (`:9090`) скрейпит все сервисы каждые 10 c
(конфиг `deployments/prometheus.yml`). Grafana (`:3001`) поднимается с
автоматическим провижинингом источника данных и дашборда «CoachLink Platform»
(`deployments/grafana/`): анонимный вход без формы логина — дашборд открывается
в один клик. Панели: RPS по сервисам, частота ошибок 5xx, p95-задержка по
маршрутам, трафик Notification Service и суммарный объём запросов за час.

Нагрузку для наполнения дашборда генерирует `make traffic`
(`scripts/gen-traffic.sh`) — смесь успешных и заведомо ошибочных (401/400)
запросов, чтобы были видны все панели.

---

## 8. Analytics Service

**Порт:** 8005 | **Язык:** Go | **БД:** нет (stateless)

**Ответственность:** агрегация данных тренировок, сводная статистика, прогресс по периодам.

**Принцип работы:** Analytics Service не имеет собственной базы данных. При каждом запросе он обращается к Training Service через внутренний HTTP API (`/internal/reports`, `/internal/athletes/{id}/stats`, `/internal/coach/{id}/overview`), агрегирует данные и возвращает результат. Это исключает проблемы синхронизации данных между сервисами.

**Эндпоинты (тренер):**

| Метод | Путь | Описание |
|-------|------|----------|
| GET | `/api/v1/analytics/athletes/{id}/summary` | Сводка по спортсмену |
| GET | `/api/v1/analytics/athletes/{id}/progress?period=week\|month` | Прогресс по периодам |
| GET | `/api/v1/analytics/overview` | Обзор по всем спортсменам |

**Эндпоинты (спортсмен):**

| Метод | Путь | Описание |
|-------|------|----------|
| GET | `/api/v1/analytics/me/summary` | Собственная сводка |
| GET | `/api/v1/analytics/me/progress?period=week\|month` | Собственный прогресс |

**Ключевые метрики:** кол-во тренировок, общая длительность, средняя нагрузка (RPE), средний/максимальный пульс, общая дистанция, процент выполнения заданий.

---

## 9. AI Service + Ollama (LLM)

**Порт:** 8006 (AI Service), 11434 (Ollama) | **Язык:** Go | **БД:** нет (stateless)

**Ответственность:** генерация текстовых рекомендаций по тренировочному процессу спортсмена с помощью локальной LLM.

### 9.1 Архитектура

```
                                   ┌→ User Service      (проверка связи тренер-спортсмен)
Тренер → API Gateway → AI Service ─┼→ Analytics Service (статистика и отчёты спортсмена)
                                   └→ Ollama            (генерация текста)
```

AI Service — stateless-оркестратор без собственной базы данных. При каждом запросе:
1. Проверяет роль (только тренер)
2. Проверяет через User Service, что спортсмен действительно связан с этим тренером
3. Получает из Analytics Service сводную статистику и последние отчёты спортсмена
4. Формирует промпт на русском языке с конкретными числовыми данными и комментариями из отчётов
5. Отправляет промпт в Ollama API (`POST /api/chat`, `stream: false`)
6. Возвращает сгенерированный текст рекомендаций

AI-модуль является вспомогательным инструментом для тренера: он помогает быстрее выделить тенденции, риски и возможные корректировки нагрузки, но не заменяет профессиональное решение тренера.

### 9.2 Рекомендации

**Эндпоинт:** `POST /api/v1/ai/athletes/{id}/recommendations`

**Источник данных:**
- сводка по спортсмену из Analytics Service: объём тренировок, длительность, дистанция, средняя нагрузка, ЧСС, процент выполнения заданий;
- последние отчёты спортсмена с описанием тренировки, длительностью, воспринимаемой нагрузкой, ЧСС и дистанцией.

Сервис использует последние отчёты спортсмена, ограничивая их количество до 5, чтобы промпт оставался компактным, а генерация не занимала слишком много времени. В типовом сценарии тренер получает рекомендации по 3-5 последним отчётам.

**Назначение:** помочь тренеру скорректировать тренировочную нагрузку конкретного спортсмена на ближайшие 1-2 недели.

**Ограничения:** если у спортсмена ещё нет отчётов, сервис возвращает ошибку недостатка данных. Если Ollama недоступна, API возвращает `503 Service Unavailable`.

### 9.3 Промпты

Промпт находится в `services/ai-service/internal/service/service.go` и состоит из двух частей:
- **Системный промпт** — роль LLM ("помощник тренера по лёгкой атлетике") и строгий формат ответа: тенденции, риски, рекомендации
- **User prompt** — конкретные данные спортсмена: числовые метрики, последние отчёты и дополнительный контекст от тренера

Промпты на русском языке — LLM отвечает на русском.

### 9.4 Ollama

[Ollama](https://ollama.com/) — это сервер для запуска LLM-моделей. Разворачивается в Docker-контейнере с кастомным entrypoint, который автоматически загружает модель при первом запуске.

**Модель по умолчанию:** `gemma3:4b` (Google Gemma 3, 4 млрд параметров, ~3 ГБ)

**Автоматическая загрузка:** при `make up` контейнер Ollama запускается, и entrypoint-скрипт проверяет наличие модели. Если модели нет — автоматически скачивает её. Модель сохраняется в Docker volume `ollama_data` и не загружается повторно при перезапусках.

**Хранение модели:** модель хранится внутри Docker volume, а не в файловой системе проекта. При `git push` ничего лишнего не попадает в репозиторий.

**Производительность:** на CPU генерация занимает 1–3 минуты. Для ускорения на Apple Silicon рекомендуется установить Ollama нативно (`brew install ollama`) и указать в docker-compose переменную `OLLAMA_URL: http://host.docker.internal:11434` для ai-service.

### 9.5 Эндпоинты (только для тренера)

| Метод | Путь | Описание |
|-------|------|----------|
| POST | `/api/v1/ai/athletes/{id}/recommendations` | Рекомендации по тренировкам спортсмена |
...
## 10. Подготовка к будущим расширениям

### 10.1 Структурированные тренировки
...
### 10.3 Расширение AI-сценариев

В текущей реализации AI Service предоставляет только рекомендации по конкретному спортсмену. В будущих версиях можно добавить отдельные сценарии:
- анализ тренировочного процесса спортсмена за выбранный период;
- сводку по отчётам всех спортсменов тренера;
- рекомендации с учётом календаря соревнований и специализации спортсмена.

---

## 11. Справочник API (краткий)
...
| GET    | /api/v1/training/groups/{groupId}/plans      | Планы, назначенные группе             |
...
| Метод | Путь                                            | Описание                         |
|-------|-------------------------------------------------|----------------------------------|
| GET   | /api/v1/analytics/athletes/{id}/summary         | Сводка по спортсмену (тренер)    |
| GET   | /api/v1/analytics/athletes/{id}/progress        | Прогресс по периодам (тренер)    |
| GET   | /api/v1/analytics/overview                      | Обзор тренера                    |
| GET   | /api/v1/analytics/me/summary                    | Собственная сводка (спортсмен)   |
| GET   | /api/v1/analytics/me/progress                   | Собственный прогресс (спортсмен) |

### AI Service

| Метод | Путь                                            | Описание                         |
|-------|-------------------------------------------------|----------------------------------|
| POST  | /api/v1/ai/athletes/{id}/recommendations        | Рекомендации от LLM (тренер)     |
