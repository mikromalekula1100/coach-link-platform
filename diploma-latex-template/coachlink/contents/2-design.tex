...
API Gateway проверяет авторизацию и перенаправляет запросы к backend-сервисам. Сервисы реализованы на Go и взаимодействуют между собой через HTTP и NATS JetStream. Для хранения данных используется PostgreSQL. Для генерации AI-рекомендаций используется локальный LLM-сервер Ollama. На рисунке~\ref{fig:components-scheme} представлена компонентная схема backend-платформы. Сплошными стрелками на схеме показаны синхронные взаимодействия: HTTP-запросы между сервисами и обращения к базам данных. Пунктирными стрелками показано асинхронное событийное взаимодействие через NATS JetStream.
...
Выбор языка Go~\cite{go-docs} в качестве основного инструмента backend-разработки обусловлен его высокой эффективностью при создании микросервисной архитектуры. К ключевым преимуществам языка в контексте решаемой задачи относятся: эффективная реализация параллелизма, высокая скорость исполнения кода и богатая экосистема модулей для маршрутизации запросов, взаимодействия с базами данных и брокерами сообщений.

Для HTTP API используется Echo~\cite{echo-docs}. Этот фреймворк предоставляет маршрутизацию, middleware, обработку запросов и удобную интеграцию с валидацией. Для доступа к PostgreSQL~\cite{postgres-docs} применяется \passthrough{\lstinline!sqlx!}, так как он позволяет сохранять контроль над SQL-запросами и при этом упрощает маппинг результатов в структуры Go. Миграции базы данных выполняются через \passthrough{\lstinline!goose!}.

Для аутентификации используется схема access/refresh токенов. Access token реализован как JWT~\cite{jwt-rfc} и содержит идентификатор пользователя, логин и роль. Пароли хранятся в виде bcrypt-хешей. Для асинхронного взаимодействия сервисов используется NATS JetStream~\cite{nats-docs}, обеспечивающий durable-подписки и доставку событий после временной недоступности сервиса.

Контейнеризация выполнена через Docker Compose~\cite{docker-docs}. Это позволяет запускать все сервисы платформы, PostgreSQL, NATS и Ollama~\cite{ollama-docs} одной командой. API документируется в формате OpenAPI~\cite{openapi-spec}, что делает backend-контракты пригодными для подключения внешних клиентов.
...
Access token реализован как JWT. В payload токена включаются поля, представленные на рисунке~\ref{src:design-jwt-payload}.
...
\end{lstlisting}
\caption{Структура payload access token}
\label{src:design-jwt-payload}
\end{figure}
...
Публичный endpoint: \lstinline!POST /api/v1/ai/athletes/{id}/recommendations!.
...
Общая структура данных представлена на ER-диаграмме (рисунок~\ref{fig:er-diagram}). Диаграмма отражает логические связи между основными сущностями платформы. В микросервисной архитектуре таблицы физически распределены по базам данных отдельных сервисов. Расширенная ER-диаграмма с детализацией полей и типов данных приведена в приложении Г.
...
Асинхронное взаимодействие реализовано через NATS JetStream. Оно используется там, где действие может быть выполнено после основного запроса и не должно блокировать пользователя. Все события платформы публикуются в единый стрим \passthrough{\lstinline!COACHLINK!} с wildcard-подпиской \passthrough{\lstinline!coachlink.>!} и файловым хранилищем, обеспечивающим сохранность сообщений при перезапуске брокера. Стрим создаётся программно при старте каждого сервиса через вызов \passthrough{\lstinline!AddStream!}, а не через внешнюю конфигурацию.

Для каждого типа события подписчик создаёт отдельный durable consumer с параметрами \passthrough{\lstinline!DeliverAll!} и \passthrough{\lstinline!AckExplicit!}, что обеспечивает гарантию доставки <<at-least-once>>: сообщение считается обработанным только после явного подтверждения (\passthrough{\lstinline!Ack!}), а при ошибке отклоняется (\passthrough{\lstinline!Nak!}) и будет доставлено повторно.

Например, при создании тренировочного назначения Training Service публикует событие \passthrough{\lstinline!coachlink.training.assigned!}. Notification Service получает событие и создаёт уведомление для спортсмена. Если Notification Service временно недоступен, durable consumer сохраняет позицию чтения, и событие будет обработано после восстановления сервиса.

В таблице~\ref{tab:nats-key-events} приведён список ключевых событий. Полный перечень событий с развёрнутым описанием назначения приведён в приложении Д.
...
\hline
\endfirsthead
\caption*{Продолжение таблицы \ref{tab:nats-key-events}}\\
\hline
Событие & Источник & Получатель & Назначение \\
\hline
\endhead
...
\hline
\endfirsthead
\caption*{Продолжение таблицы \ref{tab:public-api-groups}}\\
\hline
Группа & Назначение \\
\hline
\endhead
...
Такое разделение упрощает маршрутизацию на уровне API Gateway и позволяет документировать каждую группу endpoint независимо. При проектировании API соблюдались следующие принципы: версионирование через префикс \passthrough{\lstinline!/v1/!}, что позволяет вводить несовместимые изменения в будущих версиях без нарушения работы существующих клиентов; единообразный формат ошибок с HTTP-статусом и JSON-телом, содержащим описание ошибки; пагинация списков через query-параметры \passthrough{\lstinline!limit!} и \passthrough{\lstinline!offset!}; использование стандартных HTTP-методов в соответствии с семантикой REST. Полная спецификация API оформлена в формате OpenAPI и доступна в репозитории проекта. Примеры endpoint каждой группы приведены в приложении Б.
...
Спроектированная архитектура охватывает все функциональные требования, сформулированные в аналитической главе: она поддерживает ролевую модель, связи тренера и спортсмена, групповые назначения, стандартизированные отчёты, уведомления и аналитику, а каждая предметная область закреплена за отдельным сервисом. При этом система остаётся расширяемой: текущая реализация ориентирована на лёгкую атлетику, но базовая модель может быть адаптирована для других видов спорта. Соответствие конкретных требований реализующим их endpoint и подтверждающим тестам приводится в главе реализации после описания API и тестирования.
