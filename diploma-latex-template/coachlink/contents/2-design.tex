...
\bottomrule\noalign{}
\endlastfoot
\passthrough{\lstinline!users!} & Учётные данные пользователя, роль и пароль в виде bcrypt-хеша \\
\passthrough{\lstinline!refresh\_tokens!} & Хеши refresh-токенов и срок их действия \\
\end{longtable}
...
\bottomrule\noalign{}
\endlastfoot
\passthrough{\lstinline!user\_profiles!} & Профили пользователей, синхронизированные из Auth Service \\
\passthrough{\lstinline!connection\_requests!} & Заявки спортсмена тренеру \\
\passthrough{\lstinline!coach\_athlete\_relations!} & Активные связи тренер-спортсмен \\
\passthrough{\lstinline!training\_groups!} & Тренировочные группы тренера \\
\passthrough{\lstinline!training\_group\_members!} & Состав тренировочных групп \\
\end{longtable}
...
\bottomrule\noalign{}
\endlastfoot
\passthrough{\lstinline!training\_plans!} & Общая информация о тренировочном плане \\
\passthrough{\lstinline!training\_assignments!} & Назначение плана конкретному спортсмену \\
\passthrough{\lstinline!training\_reports!} & Отчёт спортсмена по назначению \\
\passthrough{\lstinline!training\_templates!} & Шаблоны тренировок тренера \\
\end{longtable}
...
\bottomrule\noalign{}
\endlastfoot
\passthrough{\lstinline!notifications!} & Уведомления пользователей \\
\passthrough{\lstinline!device\_tokens!} & FCM-токены устройств \\
\end{longtable}
...
\bottomrule\noalign{}
\endlastfoot
\passthrough{\lstinline!coachlink.user.registered!} & Auth Service & User Service & Создание профиля \\
\passthrough{\lstinline!coachlink.connection.requested!} & User Service & Notification Service & Уведомление тренера \\
\passthrough{\lstinline!coachlink.connection.accepted!} & User Service & Notification Service & Уведомление спортсмена \\
\passthrough{\lstinline!coachlink.connection.rejected!} & User Service & Notification Service & Уведомление спортсмена \\
\passthrough{\lstinline!coachlink.group.athlete\_added!} & User Service & Notification Service & Уведомление спортсмена \\
\passthrough{\lstinline!coachlink.group.athlete\_removed!} & User Service & Notification Service & Уведомление спортсмена \\
\passthrough{\lstinline!coachlink.training.assigned!} & Training Service & Notification Service & Уведомление спортсмена \\
\passthrough{\lstinline!coachlink.training.deleted!} & Training Service & Notification Service & Уведомление спортсмена \\
\passthrough{\lstinline!coachlink.report.submitted!} & Training Service & Notification Service & Уведомление тренера \\
\end{longtable}
...
\bottomrule\noalign{}
\endlastfoot
\passthrough{\lstinline!/api/v1/auth/*!} & Регистрация, вход, обновление и отзыв токенов \\
\passthrough{\lstinline!/api/v1/users/*!} & Профиль текущего пользователя и поиск \\
\passthrough{\lstinline!/api/v1/connections/*!} & Заявки и связи тренер-спортсмен \\
\passthrough{\lstinline!/api/v1/groups/*!} & Тренировочные группы \\
\passthrough{\lstinline!/api/v1/training/*!} & Планы, назначения, отчёты, шаблоны \\
\passthrough{\lstinline!/api/v1/notifications/*!} & Уведомления \\
\passthrough{\lstinline!/api/v1/analytics/*!} & Аналитика тренировок \\
\passthrough{\lstinline!/api/v1/ai/*!} & AI-рекомендации \\
\end{longtable}
