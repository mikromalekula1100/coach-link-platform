...
func main() {
	...
	cfg := config.Load()
	logger.Info().Str("port", cfg.AppPort).Msg("starting notification-service")
	...
	// ── NATS Consumers ─────────────────────────
	cm := consumer.NewConsumerManager(js, svc, logger)
	if err := cm.Start(); err != nil {
		logger.Fatal().Err(err).Msg("failed to start NATS consumers")
	}
	defer cm.Stop()
