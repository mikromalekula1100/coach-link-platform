...
func main() {
	...
	logger := zerolog.New(os.Stdout).With().Timestamp().Str("service", "ai-service").Logger()
	log.Logger = logger

	cfg := config.Load()
	logger.Info().Str("port", cfg.AppPort).Msg("starting ai-service")
	...
	logger.Info().Msg("ai-service stopped")
}
